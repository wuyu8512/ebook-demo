...
<div class="ebook">
  ...
  <menu-bar :isMenuShow="isMenuShow" :fontSizeList="fontSizeList" :defaultFontSize="defaultFontSize" ref="menuBar" @setFontSize="setFontSize" :themeList="themeList" :defaultTheme="defaultTheme" @setTheme="setTheme" :bookAvailabel="bookAvailabel" @onProgressChange="onProgressChange"
  :nav="nav" @jumpTo="jumpTo" @hideTitle="hideTitle"></menu-bar>
</div>
</template>
  ...
  methods: {
    ...
    showEpub (book) {
      ...
      this.rendition.on('click', (e) => {
        ...
        if (e.x % window.innerWidth > window.innerWidth * 0.75) {
          ...
          console.log('上一页')
          this.hide()
        } else if (e.clientY < window.innerHeight * 0.75 && e.clientY > window.innerHeight * 0.25) {
      ...
      this.book.ready.then(() => {
        ...
        setTimeout(function () { myVue.$refs.titleBar.hide() }, 2000)
      })
    },
    ...
    hide () {
      ...
      if (myVue.bookAvailabel) {
        myVue.$refs.menuBar.setProgress(parseInt(myVue.locations.percentageFromCfi(myVue.rendition.location.start.cfi) * 1000))
      }
    },
    hideTitle () {
