...
})(typeof self !== 'undefined' ? self : this, function(__WEBPACK_EXTERNAL_MODULE_46__, __WEBPACK_EXTERNAL_MODULE_76__) {
...
function parse(markup, mime, forceXMLDom) {
	...
	if (typeof DOMParser === "undefined" || forceXMLDom) {
		Parser = __webpack_require__(46).DOMParser;
	} else {
		Parser = DOMParser;
	}
  ...
  */
	...
	scrolledLocation() {
		...
		var sections = visible.map(view => {
			...
			var totalPages = this.layout.count(height, pageHeight).pages;
			...
			var mapping = this.mapping.page(view.contents, view.section.cfiBase, startPos, endPos);
	...
	paginatedLocation() {
		...
		var sections = visible.map(view => {
			...
			var end = start + this.layout.width - used;

			var mapping = this.mapping.page(view.contents, view.section.cfiBase, start, end);
			...
			var totalPages = this.layout.count(width).pages;
			var startPage = Math.floor(start / this.layout.pageWidth);
			var pages = [];
			var endPage = Math.floor(end / this.layout.pageWidth);
...
var _themes = __webpack_require__(58);
	...
	adjustImages(contents) {
		...
		contents.addStylesheetRules({
			"img": {
				...
				//"max-width": (this._layout.columnWidth - parseFloat(computed.paddingLeft) - parseFloat(computed.paddingRight)) + "px" + "!important",
	...
	handleLinks(contents) {
		if (contents) {
			contents.on(_constants.EVENTS.CONTENTS.LINK_CLICKED, href => {
				var relative = this.book.path.relative(href);
...
var _marksPane = __webpack_require__(60);
...
class ContinuousViewManager extends _default2.default {
	...
	check(_offsetLeft, _offsetTop) {
		...
		var promises = newViews.map(view => {
	...
	addEventListeners(stage) {
		...
		if (this.isPaginated && this.settings.snap) {
			this.snapper = new _snap2.default(this, this.settings.snap && typeof this.settings.snap === "object" && this.settings.snap);
		}
	}
...
var _rendition = __webpack_require__(19);
...
var _urlPolyfill = __webpack_require__(80);
...
var _continuous = __webpack_require__(25);
...
var _spine = __webpack_require__(47);

var _spine2 = _interopRequireDefault(_spine);

var _locations = __webpack_require__(51);
...
var _packaging = __webpack_require__(53);
...
var _pagelist = __webpack_require__(56);
...
var _archive = __webpack_require__(75);
...
var _displayoptions = __webpack_require__(79);
...
class Book {
	...
	unpack(packaging) {
		...
		if (this.packaging.metadata.layout === "") {
			...
			this.load(this.url.resolve(IBOOKS_DISPLAY_OPTIONS_PATH)).then(xml => {
				this.displayOptions = new _displayoptions2.default(xml);
  ...
  */
	loadNavigation(packaging) {
		...
		if (toc) {
			return new Promise((resolve, reject) => {
				this.navigation = new _navigation2.default(toc);

				if (packaging.pageList) {
					this.pageList = new _pagelist2.default(packaging.pageList); // TODO: handle page lists from Manifest
				}
  ...
  */
	store(name) {
		...
		this.opened.then(() => {
			...
			// Substitute hook
			var substituteResources = (output, section) => {
...
var isValue         = __webpack_require__(17)
  , isPlainFunction = __webpack_require__(29)
...
module.exports = __webpack_require__(43)() ? String.prototype.contains : __webpack_require__(44);
...
var _section = __webpack_require__(48);
	...
	render(_request) {
		...
		this.load(_request).then(function (contents) {
			...
			if (typeof XMLSerializer === "undefined" || isIE) {
				Serializer = __webpack_require__(49).XMLSerializer;
			} else {
...
//if(typeof require == 'function'){
	var XMLReader = __webpack_require__(50).XMLReader;
...
class Themes {
  ...
  */
	override(name, value, priority) {
		...
		contents.forEach(content => {
			content.css(name, value, priority);
		});
	}
  ...
  */
	font(f,priority) {
		this.override("font-family", f, priority)
	}
...
var _throttle = __webpack_require__(64);
	...
	checkRequirements() {
		try {
			if (typeof JSZip === "undefined") {
				var _JSZip = __webpack_require__(76);
				this.zip = new _JSZip();
			} else {
				this.zip = new JSZip();
			}
...
if(typeof __WEBPACK_EXTERNAL_MODULE_76__ === 'undefined') {var e = new Error("Cannot find module \"jszip\""); e.code = 'MODULE_NOT_FOUND'; throw e;}
...
class Store {
  ...
  */
	add(resources, force) {
		var mapped = resources.resources.map(item => {
			var href = item.href;

			var url = this.resolver(href);
